...
parcelHelpers.export(exports, "indentBlock", ()=>indentBlock
);
parcelHelpers.export(exports, "unindentBlock", ()=>unindentBlock
);
parcelHelpers.export(exports, "blockIndentationExtension", ()=>blockIndentationExtension
);
var _basicSetup = require("@codemirror/basic-setup");
var _state = require("@codemirror/state");
var _view = require("@codemirror/view");
const baseTheme = _basicSetup.EditorView.baseTheme({
    ".cm-block-indentation": {
        paddingLeft: "3rem"
    }
});
const blockIndendationDecoration = _view.Decoration.line({
    attributes: {
        class: "cm-block-indentation"
    }
});
const increaseBlockIndentation = _state.StateEffect.define();
const decreaseBlockIndentation = _state.StateEffect.define();
const blockIndentationField = _state.StateField.define({
    create () {
        ...
        for (const effect of transaction.effects){
            if (effect.is(increaseBlockIndentation)) {
                ...
                indentations = indentations.update({
                    add: [
                        blockIndendationDecoration.range(effect.value)
                    ]
                });
            }
    ...
    provide: (f)=>_basicSetup.EditorView.decorations.from(f)
});
function indentBlock(view1) {
    let effects = view1.state.selection.ranges.map((range)=>increaseBlockIndentation.of(view1.state.doc.lineAt(range.from).from)
    );
    if (!effects.length) return false;
    if (!view1.state.field(blockIndentationField, false)) effects.push(_state.StateEffect.appendConfig.of([
        blockIndentationField,
        baseTheme
    ]));
    view1.dispatch({
        effects
...
function unindentBlock(view2) {
    let effects = view2.state.selection.ranges.map((range)=>decreaseBlockIndentation.of(view2.state.doc.lineAt(range.from).from)
    );
    if (!effects.length) return false;
    if (!view2.state.field(blockIndentationField, false)) effects.push(_state.StateEffect.appendConfig.of([
        blockIndentationField,
        baseTheme
    ]));
    view2.dispatch({
...
const indendationKeymap = _view.keymap.of([
    {
        key: "Tab",
        preventDefault: true,
        run: (view3)=>{
...
function blockIndentationExtension(_options = {
}) {
    return [
        baseTheme,
        indendationKeymap
...
},{"@codemirror/basic-setup":"k6acs","@codemirror/state":"ipfYn","@codemirror/view":"y4Gw8","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"k6acs":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@codemirror/view":"y4Gw8","@codemirror/state":"ipfYn","@codemirror/tooltip":"6Q73g","@codemirror/panel":"jUPhp","@codemirror/gutter":"cnUiu","@codemirror/rangeset":"1JOok","crelt":"hJl4u","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}]},["hMwtF","7PGg5"], "7PGg5", "parcelRequire919c")
